...
def extract_latent_embeddings(
    ...
    """Passes entity records through trained Contrastive VAE to produce normalized latent z vectors."""
...
def build_faiss_index_and_search(
    ...
    """Builds country-partitioned FAISS index over S2+S3 candidates and queries top-k candidates for each S1 entity."""
    candidate_map = {s1_id: [] for s1_id in s1_ids}
    
    # Map candidates by country
...
def run_blocking_pipeline(
    ...
    """Runs end-to-end blocking pipeline and exports candidate_pairs.tsv."""
    ...
    print(f"Loading Contrastive VAE checkpoint from {checkpoint_path}...")
    model = ContrastiveVAE(
